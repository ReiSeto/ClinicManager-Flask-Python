...
from ClinicManagerApp.controller.doctor.medical_examination_controller import get_customer_id_card as gcid, \
    ...
    save_medicine_examination as sme, get_customer_name_by_id_card, get_customer_phone_by_id_card
...
class MedicalExaminationCreationView(BaseView):
    @expose('/')
    def index(self):
        return self.render('doctor/medical_examination_creation.html', daily_customer_list=get_customer_daily_list())
...
@app.route('/api/doctor/customer_phone', methods=['post'])
def get_customer_phone():
    id_card = request.json.get('id_card')
    customer_phone = get_customer_phone_by_id_card(id_card)
    return jsonify({
        'result': '{}'.format(customer_phone[1:])
    })
